...
export default async function EstoquePage({
  ...
  const [pecas, total, todasPecas] = await Promise.all([
    prisma.peca.findMany({ orderBy: { nome: "asc" }, skip, take }),
    prisma.peca.count(),
    prisma.peca.findMany({ select: { nome: true, quantidadeAtual: true, quantidadeMin: true } }),
  ]);
  const emFalta = todasPecas.filter((p) => p.quantidadeAtual <= p.quantidadeMin);
            ...
            <tbody>
              {pecas.map((p) => {
                ...
                return (
                  <tr
                    key={p.id}
                    className={`border-b border-base-800 transition-colors ${
                      baixo ? "bg-signal/10 hover:bg-signal/20" : "hover:bg-base-800/50"
                    }`}
                  >
